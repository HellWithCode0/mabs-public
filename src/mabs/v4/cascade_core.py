"""CASCADE core: config, costs, pair/clique/peel helpers (v4.2 FLASH + FULL)."""
...
class CASCADEConfig:
    ...
    Default ``mode="flash"``: minimal control plane (empty / K=1 boundary LUT /
    K=2 pair LUT / K>=3 blossom + sticky). Opt-in ``mode="full"`` (or
    ``use_flash=False``) restores 4.1 ExactPatternCache / peel / clique / cost
    ...
    # --- FLASH (default) ---
    mode: str = "flash"  # "flash" | "full"
    use_flash: Optional[bool] = None  # None → derived from mode
    sticky_blossom: bool = False
    fill_pair_lut_on_miss: bool = True
    prewarm_boundary: bool = True
    ...
    use_cache: bool = True  # FULL only; FLASH skips ExactPatternCache
    use_clique: bool = True  # FULL only
    ...
    cost_aware: bool = True  # FULL only
    ...
    def is_flash(self) -> bool:
        ...
        return str(self.mode).lower() != "full"
    ...
    def resolved_pair_radius(self, d: int) -> int:
        if self.prewarm_pair_radius is not None:
            return max(0, int(self.prewarm_pair_radius))
...
class CASCADEState:
    ...
    # Sticky blossom: after first escalate this shot, remaining → blossom.
    sticky_this_shot: bool = False
    cache: ExactPatternCache = field(
        default_factory=lambda: ExactPatternCache(max_size=10_000, admit_after=2)
    )

    def reset_shot_sticky(self) -> None:
        self.sticky_this_shot = False
    ...
    def sticky_rate(self) -> float:
        return 0.0 if self.n_windows == 0 else self.n_sticky / self.n_windows
...
def prewarm_cascade_graphs(
    ...
    *,
    ...
    d = bundle.d
    pair_r = cfg.resolved_pair_radius(d) if cfg.is_flash() else 0
    for wm in windows:
        g = _get_graph_cache(wm)
        get_pair_lut(g)
        if cfg.is_flash():
            if cfg.prewarm_boundary or pair_r > 0:
                prewarm_flash_lut(wm, pair_radius=pair_r if cfg.prewarm_boundary else 0)
                if not cfg.prewarm_boundary:
                    # still attach empty lut
                    get_flash_lut(wm)
            else:
                get_flash_lut(wm)
