...
__all__ = [
    "WindowRecord", "CircuitBundle", "ShotDecodeResult",
    "build_surface_code_bundle", "stream_shot", "sample_syndromes",
    "sample_syndromes_with_observables", "batch_decode_observables",
    "ensure_window_schedule", "get_window_matcher", "make_window_spec",
]
...
def stream_shot(
    bundle: CircuitBundle, syndrome: np.ndarray, *,
    window_depth: int, commit_stride: int,
    kernel_name: str = "auto", auto_threshold: int = 64,
    campaign: int = 0, shot: int = 0,
...
def sample_syndromes(bundle, shots, seed):
...
def sample_syndromes_with_observables(bundle, shots, seed):
    sampler = bundle.circuit.compile_detector_sampler(seed=seed)
...
def batch_decode_observables(bundle, syndromes):
